...
# SG: This change will allow the code to run without any errors. However this change creates a setter method for both `@quanity` and `@product_name`. It seems we dont need a product_name setter at all, and it seems that `update_quantity` is already serving as a custom setter for `@quanity` so it would be better to change `line 12` to `@quanity = ...`
